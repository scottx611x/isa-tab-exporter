...
def create_descriptor():
    """Returns a simple but complete ISA-Tab 1.0 descriptor for
    illustration."""
    ...
    investigation.description = (
        ...
        "set some default "
        "values at the time of creation, however we want to demonstrate how "
        "to use the "
    ...
    # Create an empty Study object and set some values. The Study must have
    # a filename, otherwise when we serialize it
    # to ISA-Tab we would not know where to write it. We must also attach
    # the study to the investigation by adding it
    ...
    study.description = (
        ...
        "but have chosen to demonstrate in this example the use of instance "
        "variables to set initial "
        "values."
    )
    ...
    # objects. For example, a Study can have a
    # list of design descriptors. A design descriptor is an Ontology
    # Annotation describing the kind of study at hand.
    ...
    # object instance. To ensure the OntologySource
    # is encapsulated in the descriptor, it is added to a list of
    # 'ontology_source_references' in the Investigation
    # object. The 'intervention_design' object is then added to the list of
    # 'design_descriptors' held by the Study
    # object.
    ...
    # Other instance variables common to both Investigation and Study
    # objects include 'contacts' and 'publications',
    ...
    # To create the study graph that corresponds to the contents of the
    # study table file (the s_*.txt file), we need
    # to create a process sequence. To do this we use the Process class and
    # attach it to the Study object's
    # 'process_sequence' list instance variable. Each process must be linked
    #  with a Protocol object that is attached to
    # a Study object's 'protocols' list instance variable. The sample
    # collection Process object usually has as input
    ...
    # Then we create three Sample objects, with organism as Homo Sapiens,
    # and attach them to the study. We use the utility function
    # batch_create_material() to clone a prototype material object. The
    # function automatically appends
    # an index to the material name. In this case, three samples will be
    ...
    # collection protocol, and attach it to the
    # study object. Protocols must be declared before we describe Processes,
    #  as a processing event of some sort
    # must execute some defined protocol. In the case of the class model,
    # Protocols should therefore be declared
    ...
    # we are describing a sample collection
    # process that takes one source material, and produces three different
    # samples.
    #
    ...
    # study-level, and for each sample we create an
    # extraction process and a sequencing process. The extraction process
    # takes as input a sample material, and produces
    # an extract material. The sequencing process takes the extract material
    #  and produces a data file. This will
    ...
    for i, sample in enumerate(study.samples):
        ...
        # extraction process takes as input a sample, and produces an
        # extract material as output
        ...
        # Ensure Processes are linked forward and backward. plink(
        # from_process, to_process) is a function to set
        # these links for you. It is found in the isatools.model package

        plink(extraction_process, sequencing_process)

        # make sure the extract, data file, and the processes are attached
        # to the assay
